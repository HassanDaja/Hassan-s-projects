...
def login():
    def depwindow():
        accountbalance = file_data[4]
        intacc = int(accountbalance)

        depositeamount = temp_Depositeamount.get()
        if depositeamount > 0:
            Newbalance = intacc + depositeamount
            new_file = open(f'C:/Users/GTX/Desktop/accounts/{file_data[0]}', 'w')
            new_file.write(file_data[0] + '\n')
            new_file.write(file_data[1] + '\n')
            new_file.write(file_data[2] + '\n')
            new_file.write(file_data[3] + '\n')
            new_file.write('%d' % Newbalance+ '\n')
            new_file.close()
    ...
    def deposite():
        ...
        Button(depostie_screen, text="Deposite", command=depwindow, border=0, bg='#994422', fg="white", font=('consolas', 12),width=25).place(x=35, y=100)
    ...
    def withwindow():
        ...
        if withdrawamount > 0:
            if withdrawamount<=intacc:
                Newbalance = intacc - withdrawamount
                new_file = open(f'C:/Users/GTX/Desktop/accounts/{file_data[0]}', 'w')
                new_file.write(file_data[0] + '\n')
                new_file.write(file_data[1] + '\n')
                new_file.write(file_data[2] + '\n')
    ...
    def withdraw():
        ...
        Button(with_screen, text="Withdraw", command=withwindow, border=0, bg='#994422', fg="white", font=('consolas', 12),width=25).place(x=35, y=100)
    ...
    def show2():
        ...
        strname=''.join(file_data[0])
        strage=''.join(file_data[2])
        strgen=''.join(file_data[3])
        stramout=''.join(file_data[4])
        intam=int(stramout)
        letter=StringVar()
        if strgen=='Male':
            letter='M'
        ...
        customer =Bank(strname,strage,letter,intam)
    ...
    def show():
        info_screen=Toplevel(login_screen)
        info_screen.title("Screen register")
        customer=(file_data[0],file_data[2],file_data[3])
        Label(info_screen,text=customer.show_details(),font=('calibri',14)).place(x=100,y=100)
        mainloop()
    def des1():
        window.deiconify()
        loginsuc.withdraw()
    ...
    def loginbutton():
        ...
        global stat1
        stat1 = BooleanVar()
        for acc1 in os.listdir('C:/Users/GTX/Desktop/accounts'):

            if loginuser==acc1:
                stat1 = True
                file= open(f'C:/Users/GTX/Desktop/accounts/{loginuser}','r')
                global file_data
                file_data=file.read()
                ...
                if loginpass==file_data[1]:
                    ...
                    Label(loginsuc,text=f'Welcome Back {loginuser},We hope \n you are doing great',font=('consolas',14),bg='#211120',fg='white').place(x=60,y=30)
                    Button(loginsuc,text='Show User Details',command=show2,border=0,bg='#994422',fg="white",font=('consolas',12),width=25).place(x=60,y=110)
                    Button(loginsuc, text='Deposit',command=deposite,border=0,bg='#994422',fg="white",font=('consolas',12),width=25).place(x=60,y=160)
                    Button(loginsuc, text='WithDraw',command=withdraw,border=0,bg='#994422',fg="white",font=('consolas',12),width=25).place(x=60,y=210)
                    Button(loginsuc, text='LOGOUT',command=des1,border=0,bg='#994422',fg="white",font=('consolas',12),width=25).place(x=60,y=260)
    ...
    Button(login_screen,text='L O G I N',command=loginbutton,fg='white',bg='#994422',border=0).place(x=140,y=360)
...
def register():
    ...
    def finish():
        ...
        for acc in os.listdir('C:/Users/GTX/Desktop/accounts'):
            if acc==name:
                notif.config(fg='red', text="The account already exists")
                return
                ...
                new_file = open(f'C:/Users/GTX/Desktop/accounts/{name}','w')
                new_file.write(name + '\n')
    ...
    Button(reg_screen,text='R E G I S T E R',fg='white',bg='#101429',border=0,font=('consolas', 14),command=finish).place(x=110,y=560)
    Button(reg_screen, text='<<', fg='white', bg='#101429', border=0, font=('consolas', 14),command=back).place(x=60, y=560)
    #not
...
Button(window, text='L O G I N', command=login, fg='white', bg='#994422', border=0,font=('consolas', 14),width=20).place(x=50, y=290)

#Button(window,text="Register",font=('Calibri',12),width=20,command=register).grid(row=3,sticky=N)
#Button(window,text="Login",font=('Calibri',12),width=20,command=login).grid(row=4,sticky=N,pady=10)
#sho
